...
export const datosPlantaService = {
  ...
  obtenerDatosPlanta: async (plantId, limite = 100, pagina = 1) => {
    const response = await api.get(`/api/datos-planta/planta/${plantId}?limite=${limite}&pagina=${pagina}`);
    return response.data;
  },

  // Obtener datos por rango de fechas
  obtenerDatosRangoFechas: async (plantId, fechaInicio, fechaFin) => {
    const response = await api.get(`/api/datos-planta/planta/${plantId}/rango?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`);
    return response.data;
  },
  ...
  crearDato: async (datoData) => {
    const response = await api.post('/api/datos-planta', datoData);
    return response.data;
  },
};