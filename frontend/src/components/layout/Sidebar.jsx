...
export default function Sidebar({ isCollapsed, onToggleCollapse, className = '' }) {
  ...
  // ✅ Usar utilidades de roles en lugar de cálculos manuales
  const permisos = useMemo(() => ({
    esSuperAdmin: esSuperAdmin(user),
    puedeGestionarPlantas: puedeGestionarPlantas(user),
    puedeAdministrar: puedeAdministrar(user),
    nombreRol: obtenerNombreRol(user?.rol)
  }), [user]);

  // ✅ Navegación optimizada con nuevos permisos
  const navigation = useMemo(() => [
    { 
      ...
      // ✅ Superadmin, Admin, Técnico ven Dashboard completo
      visible: user?.rol !== 'cliente' || permisos.esSuperAdmin
    },
    { 
      name: 'Plantas', 
      href: '/plantas', 
      ...
      // ✅ Solo quienes pueden gestionar plantas
      visible: permisos.puedeGestionarPlantas
    },
    { 
      name: 'Incidencias', 
      href: '/incidencias', 
    ...
    // ✅ NUEVO: Opción de Administración para Superadmin/Admin
    ...(permisos.puedeAdministrar ? [{
      name: 'Administración', 
      href: '/admin', 
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      visible: true,
      highlight: permisos.esSuperAdmin
    }] : [])
  ], [user?.rol, permisos]);
  ...
  // ✅ Efecto para sincronizar estado móvil con colapsado
  useEffect(() => {
    if (isMobile) {
  ...
  const sidebarClasses = clsx(
    ...
    isCollapsed && !isMobileOpen ? SIDEBAR_CONFIG.sizes.collapsed.width : SIDEBAR_CONFIG.sizes.expanded.width,
    className
  );

  // ✅ Obtener texto del panel según rol
  const getPanelText = useMemo(() => {
    if (permisos.esSuperAdmin) return 'Panel Super Admin';
    if (user?.rol === 'admin') return 'Panel Administrador';
    if (user?.rol === 'tecnico') return 'Panel Técnico';
    if (user?.rol === 'cliente') return 'Portal Cliente';
    return 'Gestión Plantas';
  }, [user?.rol, permisos.esSuperAdmin]);

  // ✅ Exportar función para abrir desde Header
  useEffect(() => {
    window.openSidebarMobile = handleOpenMobile;
    return () => {
      delete window.openSidebarMobile;
    };
  }, [handleOpenMobile]);
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between h-16 px-4 border-b border-secondary-100 flex-shrink-0">
            ...
            <div className={clsx(
              ...
              <div className={clsx(
                "w-8 h-8 rounded-xl flex items-center justify-center shadow-soft flex-shrink-0",
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    clsx(
                      ...
                      item.highlight && "border-l-4 border-l-purple-500"
                    )
                  }
                >
                  <div className={clsx(
          ...
          {/* Footer del Sidebar - ACTUALIZADO */}
          <div className={clsx(
                ...
                <div className="text-xs text-secondary-500 min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <div className={clsx(
                      ...
                      permisos.esSuperAdmin ? "bg-purple-500" : "bg-success-500"
                    )}></div>
                    <span className="truncate font-medium">
                      {permisos.esSuperAdmin ? 'Sistema Super Admin' : 
                       user?.rol === 'cliente' ? 'Portal activo' : 'Sistema activo'}
                    </span>
                  </div>
                  {user?.nombre && (
                    <div className="truncate text-secondary-600">
                      {permisos.esSuperAdmin ? 'Super Admin' : user.nombre}
                      {user?.rol && (
